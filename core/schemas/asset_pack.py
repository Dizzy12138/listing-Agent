...
class AssetItem(BaseModel):
    ...
    asset_pack_id: str
    name: str
    item_type: str = "icon"  # icon | arrow | doodle | decoration | scene_reference | competitor_image | logo | accessory | layout_reference | graphic | background
    type: str = "icon"  # backward-compatible alias; frontend should use item_type
    group: str = "其他"  # 产品 | 功能 | 包装 | 箭头 | 场景 | 竞品 | 品牌 | 其他
    tags: list[str] = Field(default_factory=list)
    ...
    confidence: float = 0
    source: str = "manual"  # pdf_embedded_image | pdf_page_crop | image_upload | manual | text_preview
    created_at: str = ""
