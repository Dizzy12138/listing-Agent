...
async def api_upload_pack(
    ...
    upload_dir = Path("assets/packs/uploads")
    upload_dir.mkdir(parents=True, exist_ok=True)
    ...
    pack = create_pack(pack_name, saved_files, cats, usages, file_type=ftype, pack_type=pack_type)
    return {
...
async def api_parse_pack(pack_id: str):
    ...
    if not pack:
        raise HTTPException(404, "素材包不存在")
    pack = parse_pack(pack_id)
    return {"pack_id": pack_id, "status": pack.parse_status, "pack": pack.model_dump()}
...
async def api_update_asset_item(item_id: str, body: dict):
    from core.services.asset_service import update_item
...
async def api_batch_confirm_items(body: dict):
    ...
    tags = body.get("tags", [])
    updated = batch_update_items(ids, status=status, tags=tags)
...
async def api_batch_disable_items(body: dict):
    ...
    ids = body.get("asset_item_ids", [])
    tags = body.get("tags", [])
    updated = batch_update_items(ids, status="disabled", tags=tags)
    return {"updated": len(updated), "items": updated}
...
async def api_upload_doc(
    ...
    upload_dir = Path("assets/docs/uploads")
    upload_dir.mkdir(parents=True, exist_ok=True)
...
async def api_analyze_doc(doc_id: str):
    ...
    return {"doc_id": doc_id, "status": doc.parse_status, "doc": doc.model_dump()}
